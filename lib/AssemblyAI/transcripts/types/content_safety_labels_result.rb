...
module AssemblyAI
  module Transcripts
    ...
    class ContentSafetyLabelsResult
      ...
      # @param status [Hash{String => String}] The status of the Content Moderation model. Either success, or unavailable in the rare case that the model failed.
      ...
      def initialize(status:, results:, summary:, severity_score_summary:, additional_properties: nil)
        # @type [Hash{String => String}] The status of the Content Moderation model. Either success, or unavailable in the rare case that the model failed.
        @status = status
      ...
      def self.from_json(json_object:)
        ...
        status = AUDIO_INTELLIGENCE_MODEL_STATUS.key(struct.status)
      ...
      def self.validate_raw(obj:)
        obj.status.is_a?(AUDIO_INTELLIGENCE_MODEL_STATUS) != false || raise("Passed value for field obj.status is not the expected type, validation failed.")
