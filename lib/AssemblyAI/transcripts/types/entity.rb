...
module AssemblyAI
  module Transcripts
    ...
    class Entity
      ...
      # @param entity_type [Hash{String => String}] The type of entity for the detected entity
      ...
      def initialize(entity_type:, text:, start:, end_:, additional_properties: nil)
        # @type [Hash{String => String}] The type of entity for the detected entity
      ...
      def self.from_json(json_object:)
        ...
        entity_type = ENTITY_TYPE.key(struct.entity_type)
        text = struct.text
        start = struct.start
        end_ = struct.end
      ...
      def self.validate_raw(obj:)
        obj.entity_type.is_a?(ENTITY_TYPE) != false || raise("Passed value for field obj.entity_type is not the expected type, validation failed.")
